...
# ── Config ──
MODEL_PATH     = "/content/drive/MyDrive/RFFLPA/proofs/RFFPLA_classifier.h5"
...
def predict(arrays, model):
    probs   = model.predict(np.stack(arrays), verbose=0).flatten()
...
def make_plot(bursts, probs, is_auth):
    ...
    axes[0].set_title("Amplitude envelope", color="#94a3b8",
                       fontsize=9, pad=6)
    axes[0].set_xlabel("Time (µs)", fontsize=8)
    axes[0].set_ylabel("Amplitude", fontsize=8)

    # IQ
    axes[1].plot(t, b.real, color="#3b82f6", lw=0.9,
                 label="I", alpha=0.9)
    axes[1].plot(t, b.imag, color="#22c55e", lw=0.9,
                 label="Q", alpha=0.9)
    axes[1].set_title("I / Q channels", color="#94a3b8",
                      fontsize=9, pad=6)
    axes[1].set_xlabel("Time (µs)", fontsize=8)
    leg = axes[1].legend(fontsize=8, facecolor="#1e293b",
                         edgecolor="#334155", labelcolor="#94a3b8")

    # Per-burst confidence
    n  = len(probs)
    ...
    axes[2].axhline(y=CONF_THRESHOLD, color="#f59e0b",
                    lw=1, linestyle="--", label=f"{CONF_THRESHOLD}% threshold")
...
with c1:
    ...
    <p class="sys-sub">
        Compares incoming RF signals against enrolled device fingerprint
...
with left:
    ...
    <p style="font-size:12px;color:#475569;margin-top:6px">
        5–12 second recording recommended · Max 200 MB
    </p>
    """, unsafe_allow_html=True)
    ...
    if uploaded is None:
        ...
        if not arrays:
            ...
            <div class="result-warn">
                ...
                <p class="result-sub">
                    No valid bursts detected in this file.
                    Ensure the device was transmitting during the recording.
                </p>
            </div>
            """, unsafe_allow_html=True)
        else:
            ...
            if conf < CONF_THRESHOLD:
                ...
                <div class="result-warn">
                    ...
                    <div class="meta-row">
                        <span class="meta-item">Bursts analysed: <span>{n}</span></span>
            ...
            with st.expander("Technical details"):
                ...
                | Bursts extracted | {n} |
                | Active samples (first burst) | {active} ({active/SAMPLE_RATE*1e6:.1f} µs) |
                | Individual scores | {", ".join([f"{p:.3f}" for p in probs])} |
                | Average score | {float(np.mean(probs)):.4f} |
                | Decision threshold | 0.50 (score) · {CONF_THRESHOLD}% (confidence) |
                """)
...
<p style="font-size:11px;color:#334155;text-align:center">
    ...
    HKR Bachelor Project 2026 &nbsp;·&nbsp;
    RFFPLA_Classifier (1D CNN) &nbsp;·&nbsp;
    99.69% test accuracy
</p>
""", unsafe_allow_html=True)
